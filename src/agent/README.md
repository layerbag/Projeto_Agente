...
| `tools.py` | 5 tools: `search_knowledge_base`, `web_search`, `list_documents`, `summarize_document`, `extract_document_bullets`. As tools de sumário/tópicos verificam cache na tabela `document_summaries` antes de chamar o LLM. |
...
O nó `final_response` permite que tools como `summarize_document` retornem `{"final": true, "result": "..."}` e a resposta vá direto para o usuário sem passar pelo LLM novamente.
